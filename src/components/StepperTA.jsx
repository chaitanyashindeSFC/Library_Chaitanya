...
function Stepper({ children, className = "" }) {
  return (
    <div
      className={`w-full bg-white rounded-md shadow-sm px-6 py-3 flex flex-wrap items-center gap-6 ${className}`}
    >
      {children}
    </div>
  );
...
function Step({ index, label, currentStep = 1, color = "#173B4E", totalSteps }) {
  ...
  return (
    <div className="flex items-center">
      <div className="flex items-center gap-2">
        <div
          className="flex items-center justify-center w-6 h-6 rounded-full border text-sm font-semibold transition-all duration-300"
          style={circleStyle}
        >
...
function StepConnector({ color = "#D1D5DB" }) {
  return (
...
function StepList({ steps = [], currentStep = 1, color = "#173B4E" }) {
  return (
    <>
      {steps.map((step, i) => (
        <Step
...
function StepperRoot({ steps = [], currentStep = 1, color = "#173B4E", className = "" }) {
  return (
